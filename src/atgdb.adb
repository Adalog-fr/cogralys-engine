...
--    Ada.Calendar,
  Ada.Command_Line, Ada.Exceptions, Ada.Wide_Text_IO;

-- ASIS
with Asis.Ada_Environments, Asis.Errors, Asis.Exceptions, Asis.Implementation;

-- Adalog
with Thick_Queries, Utilities, Units_List;

-- Adactl
with Atgdb_Options, Framework.Interrupt, Framework.Language, Framework.Reports,
--    Framework.Rules_Manager,
  Framework.Variables.Shared_Types;

with Export_Manager;
with Export_Manager.Neo4J;
with Export_Manager.JSONL;
...
--     use Ada.Calendar;
   ...
   use Utilities, Atgdb_Options, Units_List;
...
--     Start_Time : constant Time := Clock;
   ...
   Recursion_Table : constant array (Switch, Switch) of Recursion_Mode :=
     (Off => (Off => None, On => None), On => (Off => Full, On => Spec_Closure));
...
--     Framework.Variables.Initialize;
   ...
   if Action in Need_Asis_Actions then
      ...
      Units_List.Register
        (Units_Spec => Ada_Units_List, Recursion => Recursion_Table (Recursive_Option, Spec_Option),
         Add_Stubs  => False);
   end if;

   Export_Manager.Subscribe (Export_Manager.Neo4J.Initialize);
   --  Export_Manager.Subscribe (Export_Manager.JSONL.Initialize);
...
--           if Atgdb_Options.Debug_Option.Value = On then
...
exception
   ...
   when Occur : Asis.Exceptions.ASIS_Inappropriate_Context | Asis.Exceptions.ASIS_Inappropriate_Compilation_Unit |
     Asis.Exceptions.ASIS_Inappropriate_Element | Asis.Exceptions.ASIS_Inappropriate_Line |
     Asis.Exceptions.ASIS_Inappropriate_Line_Number =>
      Export_Manager.Finalize;
      -- Presumably, an Adactl error
      User_Message
        ("Unexpected ASIS exception at main level (" & To_Wide_String (Ada.Exceptions.Exception_Name (Occur)) & ") : " &
   ...
   when Occur : others =>
      Export_Manager.Finalize;
      User_Message
        ("Unexpected exception at main level: " & Ada.Exceptions.Wide_Exception_Name (Occur) & ' ' &
         To_Wide_String (Ada.Exceptions.Exception_Message (Occur)));
      Stack_Traceback (Occur);
      Execute ("help version;");
      Ada.Command_Line.Set_Exit_Status (Failure);
