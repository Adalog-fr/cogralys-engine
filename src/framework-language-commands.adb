...
package body Framework.Language.Commands is
   ...
   procedure Help_On_Commands is
      use Atgdb_Options, Framework.Reports;
   begin
   ...
   procedure Go_Command is
      ...
      use Atgdb_Options, Framework.Rules_Manager, Framework.Variables.Shared_Types;
      ...
      begin
         ...
         User_Message ("AdaCtl version: " & Atgdb_Version
      ...
      exception
         when Local_Occur : ASIS_Failed
            ...
            if Atgdb_Options.Exit_Option.Value = On then
               raise;
            end if;
         ...
         for I in Natural range 1 .. Units_List.Length loop
            begin
               Ruler.Process (Unit_Name  => Units_List.Current_Unit,
                              Unit_Pos   => I,
                              Spec_Only  => Atgdb_Options.Spec_Option = On,
                              Go_Count   => Go_Count);
            exception
               when Utilities.User_Error =>
   ...
   procedure Help_Command (On : in Wide_String) is
      ...
      use Asis.Implementation;
      use Atgdb_Options, Implementation_Options, Framework.Rules_Manager;
   ...
   procedure Set_Output_Command (Output_File : Wide_String; Force_Overwrite : Boolean) is
      ...
      use Atgdb_Options, Framework.Reports, Framework.String_Set, Framework.Variables.Shared_Types;
   begin
      if Action = Check or Rule_Error_Occurred then
         return;
      end if;
      ...
      if To_Upper (Output_File) = Console_Name then
         ...
         begin
            ...
            Close (Adactl_Output);
            if Force_Overwrite
              or else (Atgdb_Options.Overwrite_Option = On and not Is_Present (Seen_Files, Output_File))
            then
