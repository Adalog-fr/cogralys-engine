...
package body Framework.Ruler is
   use Framework.Locations, Framework.Plugs;

   package ACH renames Ada.Characters.Handling;

   Log      : constant Util.Log.Loggers.Logger := Util.Log.Loggers.Create ("Framework.Ruler");
   Log_Pre  : constant Util.Log.Loggers.Logger := Util.Log.Loggers.Create ("Framework.Ruler.Pre_Proc");
   Log_Post : constant Util.Log.Loggers.Logger := Util.Log.Loggers.Create ("Framework.Ruler.Post_Proc");
   Log_Is_True_Identifier : constant Util.Log.Loggers.Logger := Util.Log.Loggers.Create ("Framework.Ruler.Is_True_Identifier");
   ...
   package Processed_Units_Map_Package is new Ada.Containers.Indefinite_Hashed_Maps
     (Key_Type        => Wide_String,
      Hash            => Ada.Strings.Wide_Hash,
      Equivalent_Keys => "=",
      Element_Type    => Boolean);
   use Processed_Units_Map_Package;

   Processed_Units : Map := Empty_Map;
...
--        Framework.Specific_Plugs.Enter_Unit (Unit);
   exception
      when Occur1 : others =>
         begin
            Log.Error ("Exception "                                  --## rule line off No_Trace
                             & Exception_Name (Occur1)
                             & " in Enter_Unit for "
                             & ACH.To_String (Unit_Full_Name (Unit))
                             & " (" & Unit_Kinds'Image (Unit_Kind (Unit)) & ')');
         exception
            when Occur2 : others =>
               ...
               Log.Error ("Exception in handler for enter_unit", Occur2); --## rule line off No_Trace
         end;
         raise;
   end Enter_Unit;
      ...
      when Occur1 : others =>
         ...
         exception
            when Occur2 : others =>
               -- If we are not even able to call Unit_Full_Name (presumably), trace and ignore the exception
               -- in order to propagate the original exception (more interesting)
               Log.Error ("Exception in handler for exit_unit", Occur2); --## rule line off No_Trace
         end;
         raise;
   end Exit_Unit;
...
--        Framework.Symbol_Table.  Exit_Scope (Element);
   ...
   begin
      Log_Is_True_Identifier.Debug ("Start");
      ...
      if State.Pragma_Or_Attribute_Level = 0 then
         Log_Is_True_Identifier.Debug ("return True, because State.Pragma_Or_Attribute_Level = 0");
         return True;
      end if;

      Log_Is_True_Identifier.Debug ("Get Corresponding_Name_Definition");
      Junk := Corresponding_Name_Definition (Element);
        -- OK!
      Log_Is_True_Identifier.Debug ("return True, end");
      return True;
   exception
      when ASIS_Inappropriate_Element =>
         Log_Is_True_Identifier.Debug ("return False");
   ...
   procedure Semantic_Traverse (Unit : Asis.Compilation_Unit) is
      ...
      The_Info    : Info := (Pragma_Or_Attribute_Level          => 0,
                             Visibility                         => V_Normal,
                             Corresponding_Assignment_Statement => Asis.Nil_Element,
                             Parameters_Information             => Create_Nil_Parameters_Info);
      Duplicate   : Boolean;
   ...
   begin
      -- Assume that Standard code is already in DB
      -- TODO: to be deleted
--        if Asis.Compilation_Units.Unit_Origin (Unit) = A_Predefined_Unit then
--           return;
--        end if;
   ...
   is
      ...
      Current_State : Info;
   begin
      Framework.Plugs.Enter_Public_Part (Element);

      for Elem : Asis.Element of Visible_Part loop
         Current_State := (Pragma_Or_Attribute_Level          => State.Pragma_Or_Attribute_Level,
                           Visibility                         => V_Public,
                           Corresponding_Assignment_Statement => Asis.Nil_Element,
                           Parameters_Information             => Create_Nil_Parameters_Info);
         Semantic_Traverse_Elements (Elem, Control, Current_State);
         case Control is
            when Continue =>
               null;
            when Terminate_Immediately =>
...
--                       Rules.Uncheckable.Process_Missing_Unit ("missing proper body for " & Stub_Name);
   ...
   begin
      Log_Utilities.Debug (Log_Pre, "elt: ", Element);
      if Formal_Type_Kind (Element) = A_Formal_Private_Type_Definition then
         --  Fixme: ASIS bug occur when getting location of this node.
         --  FIXME: Following formal declarations are destroyed because of this bug
         --
         -- +===========================ASIS BUG DETECTED==============================+
         -- | ASIS 2.0.R for GNAT Community 2019 (20190517)
         --           ASIS.EXCEPTIONS.ASIS_INAPPROPRIATE_LINE_NUMBER a4g-vcheck.adb:897|
         -- | when processing Asis.Text.Lines (Element)                                |
         -- | called with A_FORMAL_PRIVATE_TYPE_DEFINITION Element                     |
         -- | (for full details see the debug image after the box)                     |
         ...
         -- +==========================================================================+
         --
         -- Example of code that create this issue:
         --
         -- generic
         --    type Source (<>) is limited private;
         --    type Target (<>) is limited private;
         --                        /\ origin of the problem, you can get it with: Type_Declaration_View on the parent.
...
--                    Framework.Specific_Plugs.Enter_Statement_List (Element);
               ...
               when An_Assignment_Statement =>
                  State.Corresponding_Assignment_Statement := Element;
                  Framework.Plugs.         Pre_Procedure (Element, State);
               when A_Procedure_Call_Statement | An_Entry_Call_Statement =>
                  State.Parameters_Information := Create_Parameters_Info (Element);
                  Framework.Plugs.         Pre_Procedure (Element, State);
                  Enter_Subprogram_Call (Element);
         ...
         when An_Expression =>
            case Expression_Kind (Element) is
               when An_Attribute_Reference =>
                  ...
                  Log_Pre.Debug ("check Is_True_Identifier:"
                     & Boolean'Image (Is_True_Identifier (Prefix (Element), State)));
                  if Is_True_Identifier (Prefix (Element), State) then
                     Framework.Plugs.         Pre_Procedure (Element, State);
                     Log_Pre.Debug ("after plugs.pre_procedure");
--                       Framework.Specific_Plugs.Pre_Procedure (Element);
                     ...
                     case Attribute_Kind (Element) is
                        when A_First_Attribute
                           ...
                           for Expr : Asis.Expression of Attribute_Designator_Expressions (Element) loop
                              Log_Utilities.Debug (Log_Pre, "Semantic traverse of:", Expr);
               ...
               when An_Identifier
                 ...
                 | An_Enumeration_Literal
                  =>
                  Log_Pre.Debug ("is an identifier or Operator Symbol or Enumeration literal");
                  declare
                     Corresponding_Instanciation : Asis.Declaration;
                  begin

                     if Is_True_Identifier (Element, State) then
                        Log_Pre.Debug ("Call Plugs.True_Identifier");
                        Framework.Plugs.         True_Identifier (Element, Corresponding_Instanciation);
                     end if;

                     Log_Pre.Debug ("after check Is_True_Identifier");

                     Framework.Plugs.       Pre_Procedure (Element,
                                                           State,
                                                           Corresponding_Instantiation => Corresponding_Instanciation);
                  end;
...
--              Framework.Specific_Plugs.Pre_Procedure (Element);
         ...
         when A_Clause =>
            case Clause_Kind (Element) is
               when A_With_Clause =>
                  -- Traverse all "A_With_Clause" unit before the current unit
                  -- This is needed to populate the database
                  for Elt : Asis.Expression of Asis.Clauses.Clause_Names (Element) loop
                     declare
                        use Asis.Compilation_Units, Asis.Expressions;
                        Good_Elt : Asis.Element;
                        CU       : Asis.Compilation_Unit;
                        UO       : Asis.Unit_Origins;
                     begin
                        case Asis.Elements.Expression_Kind (Elt) is
                           when An_Identifier =>
                              Good_Elt := Elt;
                           when A_Selected_Component =>
                              Good_Elt := Selector (Elt);
                           when An_Attribute_Reference =>
                              Good_Elt := Prefix (Elt);
                           when others =>
                              Log_Utilities.Error (Log,
                                                   "Unexpected element "
                                                   & Asis.Expression_Kinds'Image (Expression_Kind (Elt)), Elt);
                        end case;

                        CU := Enclosing_Compilation_Unit (Corresponding_Name_Definition (Good_Elt));
                        UO := Unit_Origin (CU);

                        Log_Pre.Debug ("With of this unit (" & Boolean'Image (UO = A_Predefined_Unit) & "): "
                                         & ACH.To_String (Asis.Compilation_Units.Unit_Full_Name (CU)));
                        Process (Unit_Name          => Asis.Compilation_Units.Unit_Full_Name (CU),
                                 Unit_Pos           => 0,
                                 Spec_Only          => True,
                                 Go_Count           => 0);
                     end;
...
--              Framework.Specific_Plugs.Pre_Procedure (Element);
   ...
   exception
      ...
      when Occur : others =>
         if not Unwinding_Exception then  -- Trace only first level (the rest is noise)
            begin
               Log_Utilities.Error (Log,
                                "Exception "                                  --## rule line off No_Trace
                                & Exception_Name (Occur)
                                & " in Pre_Procedure at "
                                & ACH.To_String (Safe_Image (Get_Location (Element))),
                                Element,
                                With_Source => True);
   ...
   is
      ...
      procedure Post_Subprogram is
         use Thick_Queries;
         Description : constant Call_Descriptor := Corresponding_Call_Description (Element);
      begin
         Log_Utilities.Debug (Log_Post, "Post sub-program of: ", Element);
         Log_Post.Debug ("Kind of call: ", Call_Kind'Image (Description.Kind));
         if Description.Kind /= A_Regular_Call then
            return;
         end if;
         Enter_Subprogram_Call (Element    => Element,
                                Normalized => True);
         State.Parameters_Information := Create_Parameters_Info (Call       => Element,
                                                                 Normalized => True);
         for Elt : Asis.Association of Thick_Queries.Actual_Parameters (Element    => Element,
                                                                        Normalized => True)
         loop
            Pre_Procedure (Elt, Control, State);
         end loop;
         State.Parameters_Information := Create_Nil_Parameters_Info;
      end Post_Subprogram;

   begin
      Log_Post.Debug ("Start ruler post_procedure");
      case Element_Kind (Element) is
         when A_Declaration =>
            case Declaration_Kind (Element) is
         ...
         when An_Expression =>
            case Expression_Kind (Element) is
               when A_Function_Call =>
                  ...
                  Framework.Plugs.         Post_Procedure (Element);
               when others =>
                  Log_Post.Debug ("Call Post_Procedure");
                  Framework.Plugs.         Post_Procedure (Element);
            end case;
...
--              Framework.Specific_Plugs.Post_Procedure (Element);
      ...
      Log_Post.Debug ("end of ruler Post_Procedure");
   exception
      when Utilities.User_Error =>             -- Call to Parameter_Error while traversing => propagate silently
         raise;
      when Framework.Reports.Cancellation =>   -- Too many messages while traversing => propagate silently
...
--           Process_Inhibition (My_Unit, Resume);
         ...
         when others =>
            ...
            exception
               when Occur: others =>
                  ...
                  -- in order to propagate the original exception (more interesting)
                  Log.Error ("Exception in handler for Do_Process", Occur);    --## rule line off No_Trace
            end;
            raise;
      end Do_Process;
      ...
      begin
         if Units_List.Length = 1 or Go_Count = 0 then
            return "";
         end if;
   ...
   begin -- Process
      if Processed_Units.Contains (Unit_Name) then
         -- Unit already traversed
         return;
      else
         Processed_Units.Insert (Unit_Name, False);
      end if;
...
--                 Rules.Uncheckable.Process_Missing_Unit ("missing body for " & Unit_Name);
      ...
      Processed_Units.Replace_Element (Processed_Units.Find (Unit_Name), True);
   end Process;
