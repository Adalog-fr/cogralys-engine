...
procedure Ptree_To_Dot is
   ...
   package Long_Long_Integer_Vectors is new Ada.Containers.Vectors (Index_Type   => Long_Long_Natural,
                                                                    Element_Type => Long_Long_Natural);
   use Long_Long_Integer_Vectors;

   Elt_Id   : Long_Long_Natural  := Long_Long_Natural'First;
   Parents  : Vector             := Empty_Vector;
   ...
   procedure Put_Kind (Element : in Asis.Element) is
      ...
      procedure Put_Surround (Str : Wide_String) is
      begin
         Put ("(");
         Put (Str);
         Put (")");
   ...
   begin --  Put_Kind
      case Element_Kind (Element) is
         when Not_An_Element => -- ????
            null;
         when A_Pragma =>
            Put_Surround (Pragma_Kinds'Wide_Image (Pragma_Kind (Element)));
         when A_Defining_Name =>
            Put_Surround (Defining_Name_Kinds'Wide_Image (Defining_Name_Kind (Element)));
         when A_Declaration =>
            Put_Surround (Declaration_Kinds'Wide_Image (Declaration_Kind (Element)));
         when A_Definition =>
            Put_Surround (Definition_Kinds'Wide_Image (Definition_Kind (Element)));
         when An_Expression =>
            Put_Surround (Expression_Kinds'Wide_Image (Expression_Kind (Element)));
         when An_Association =>
            Put_Surround (Association_Kinds'Wide_Image (Association_Kind (Element)));
         when A_Statement =>
            Put_Surround (Statement_Kinds'Wide_Image (Statement_Kind (Element)));
         when A_Path =>
            Put_Surround (Path_Kinds'Wide_Image (Path_Kind (Element)));
         when A_Clause =>
            Put_Surround (Clause_Kinds'Wide_Image (Clause_Kind (Element)));
   ...
   is
      ...
      function Escape_String (Src : Wide_String) return Wide_String is
         use Ada.Strings.Wide_Unbounded;
         Result : Unbounded_Wide_String := Null_Unbounded_Wide_String;
      begin
         for Elt : Wide_Character of Src loop
            if Elt = '"' then
               Result := Result & '\' & '"';
            else
               Result := Result & Elt;
...
--        end loop;
      ...
      Ada.Long_Long_Integer_Wide_Text_IO.Put (Elt_Id, Width => 0);
      Put (" [label=""");
      Put_Kind (Element);
      Put ("\n");
      declare
         Source : constant Wide_String := Escape_String (Element_Image (Element));
         Start  : Natural := 1;
         Stop   : Natural;
      ...
      begin
         ...
         if Stop = 0 then
            if Is_First_Line then
               -- Utilities.Trace("Is_First_Line 1");
...
--              New_Line;
            ...
            if Is_First_Line then
               -- Utilities.Trace("Is_First_Line 2");
   ...
   --              Put_Kind (Element);
      ...
      if not Parents.Is_Empty then
         Ada.Long_Long_Integer_Wide_Text_IO.Put (Parents.Last_Element, Width => 0);
         Put (" -> ");
         Ada.Long_Long_Integer_Wide_Text_IO.Put (Elt_Id, Width => 0);
         Put (";");
         New_Line;
...
begin  -- Ptree
   ...
   Ada.Wide_Text_IO.Put_Line ("digraph AST_Example {");
   Ada.Wide_Text_IO.Put_Line ("node [style=rounded, shape=box]");
