...
//OPTIONAL MATCH normalVar = (v)-[:IS_ENCLOSED_IN*]->(decl)
//  WHERE NOT (decl:A_GENERIC_PACKAGE_DECLARATION)

WITH *, {
MATCH (v)-[:IS_ENCLOSED_IN*]->(decl)
  WHERE NOT (decl:A_GENERIC_PACKAGE_DECLARATION)
RETURN v
} as normalVar

// Match any variable declared inside a generic package (opposite of last query)
OPTIONAL MATCH genVar = (v)-[:IS_ENCLOSED_IN*]->(:A_GENERIC_PACKAGE_DECLARATION)

// For normal variable, check if it is read and write

with *, collect(normalVar) as normals, collect(genVar) as generics

// Find read variables
CALL {
WITH normals
UNWIND normals as list
WITH nodes(list) as elts
WITH elts[0] as v
...
RETURN cu as Compilation_Unit, l as Location, v as Variable, isWrite1 AS isWrite, isRead1 as isRead
